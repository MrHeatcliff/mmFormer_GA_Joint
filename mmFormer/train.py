...
setup(args, 'training')
# Modify the transforms to handle 3 inputs (volume, segmentation, GA segmentation)
args.train_transforms = 'Compose([RandCrop3D((128,128,128)), RandomRotion(10), RandomIntensityChange((0.1,0.1)), RandomFlip(0), NumpyType((np.float32, np.int64, np.int64)),])'
args.test_transforms = 'Compose([NumpyType((np.float32, np.int64, np.int64)),])'
...
def main():
    ...
    # Load GA segmentation masks using same loader
    # ga_segment_path = os.path.join(args.datapath, 'GA_segment')
    # if not os.path.exists(ga_segment_path):
    #     logging.error(f"GA segmentation path does not exist: {ga_segment_path}")
    #     raise FileNotFoundError(f"GA segmentation path not found: {ga_segment_path}")
    
    # logging.info(f"Loading GA segmentation data from: {ga_segment_path}")
    # ga_set = Brats_loadall_GAseg_nii(transforms=args.train_transforms,
    #                           root=ga_segment_path, 
    #                           num_cls=num_cls,
    ...
    # ga_loader = MultiEpochsDataLoader(
    #     dataset=ga_set,
    #     batch_size=args.batch_size,
    #     num_workers=8,
    #     pin_memory=True,
    #     shuffle=False,  # Keep same order as train_loader
    #     worker_init_fn=init_fn)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(iter_per_epoch):
            ...
            try:
                ...
                # ga_data = next(ga_iter)
            except:
                train_iter = iter(train_loader)
                # ga_iter = iter(ga_loader)
                data = next(train_iter)
                # ga_data = next(ga_iter)
            x, target, ga_mask, mask = data[:4]
            # ga_mask = ga_data[0]
            x = x.cuda(non_blocking=True)
            target = target.cuda(non_blocking=True)
            mask = mask.cuda(non_blocking=True)
            ga_mask = ga_mask.cuda(non_blocking=True)

            model.module.is_training = True
            fuse_pred, sep_preds, prm_preds = model(x, mask, ga_mask)

...
class Transformer(nn.Module):
    ...
    def forward(self, x, pos, mask=None):
        for j in range(self.depth):
            x = x + pos + mask if mask is not None else x + pos  #new change by Dat
...
class ResNet18_3D(nn.Module):
    def __init__(self, block, num_blocks):
        ...
        self.conv1 = nn.Conv3d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)  # 128 -> 64
        ...
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2)  # 32 -> 16
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)  # 16 -> 8
    ...
    def forward(self, x):
        x = F.relu(self.bn1(self.conv1(x)))  # (B, 64, 64, 64, 64)
        x = self.maxpool(x)                  # (B, 64, 32, 32, 32)
        x = self.layer1(x)                   # (B, 64, 32, 32, 32)
        x = self.layer2(x)                   # (B, 128, 16, 16, 16)
        x = self.layer3(x)                   # (B, 256, 8, 8, 8)
        return x
    
class GAMaskExtractorResNet18_3D(nn.Module):
    def __init__(self, embed_dim=transformer_basic_dims):
        super(GAMaskExtractorResNet18_3D, self).__init__()
        self.resnet = ResNet18_3D(BasicBlock3D, [2, 2, 2, 2])
        # Patch Embedding
        self.proj = nn.Conv3d(256, embed_dim, kernel_size=1, stride=1)

    def forward(self, mask):
        # mask: (B, 1, 128, 128, 128)
        x = self.resnet(mask)       # (B, 256, 8, 8, 8)
        x = self.proj(x)            # (B, 512, 8, 8, 8)
        x = x.flatten(2)            # (B, 512, 512) - 8*8*8 = 512 patches
        x = x.transpose(1, 2)       # (B, 512, 512)
        return x
# End change
#############################################
...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        ##########################################################################
        # New change by Dat
        self.mask_flair = GAMaskExtractorResNet18_3D(embed_dim=transformer_basic_dims)
        self.mask_t1ce = GAMaskExtractorResNet18_3D(embed_dim=transformer_basic_dims)
        # self.mask_t1 = self.mask_t1ce
        # self.mask_t2 = self.mask_flair
        ##########################################################################
    ...
    def forward(self, x, mask, GAMask):
        ...
        ##########################################################################
        # New change by Dat
        mask_flair = self.mask_flair(GAMask[:, 0:1, :, :, :])
        mask_t1ce = self.mask_t1ce(GAMask[:, 1:2, :, :, :])
        mask_t1 = mask_t1ce
        mask_t2 = mask_flair
        ##########################################################################
        ...
        ##########################################################################
        # New change by Dat
        flair_intra_token_x5 = self.flair_transformer(flair_token_x5, self.flair_pos, mask_flair)
        t1ce_intra_token_x5 = self.t1ce_transformer(t1ce_token_x5, self.t1ce_pos, mask_t1ce)
        t1_intra_token_x5 = self.t1_transformer(t1_token_x5, self.t1_pos, mask_t1)
        t2_intra_token_x5 = self.t2_transformer(t2_token_x5, self.t2_pos, mask_t2)
        ##########################################################################
        ...
        ############################################################################################################
        # New change by Dat
        multimodal_mask = torch.cat((mask_flair, mask_t1ce, mask_t1, mask_t2), dim=1)
        ############################################################################################################
        multimodal_inter_token_x5 = self.multimodal_transformer(multimodal_token_x5, multimodal_pos, multimodal_mask)
